...
import { Filter, Heart, ShoppingCart, Eye, SlidersHorizontal, X } from "lucide-react";
...
// Inject shared premium styles once
if (typeof document !== "undefined" && !document.getElementById("pp-anim")) {
  ...
  s.textContent = `
    ...
    * { font-family: 'Inter', sans-serif; box-sizing: border-box; }
    ...
    .pp-btn-icon:hover { transform:scale(1.08); }
    .pp-btn-icon { transition:transform .2s; }
  `;
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#fafafa" }}>
      ...
      <div style={{
        ...
        background:"linear-gradient(135deg, #1c1c2e 0%, #2d2b55 50%, #1a1a2e 100%)",
        padding:"72px 24px 80px", marginBottom:0,
      }}>
        <div style={{ position:"absolute", top:-80, right:-80, width:360, height:360, background:"radial-gradient(circle,rgba(139,92,246,.2) 0%,transparent 70%)", borderRadius:"50%", pointerEvents:"none" }} />
        <div style={{ position:"absolute", bottom:-60, left:-60, width:280, height:280, background:"radial-gradient(circle,rgba(99,102,241,.15) 0%,transparent 70%)", borderRadius:"50%", pointerEvents:"none" }} />
        <div style={{ maxWidth:1200, margin:"0 auto", position:"relative", zIndex:1 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(139,92,246,.15)", border:"1px solid rgba(139,92,246,.3)", color:"#c4b5fd", padding:"6px 16px", borderRadius:999, fontSize:12, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", marginBottom:20 }}>
            ✦ Exclusive Collection
          </div>
          <h1 style={{ fontSize:"clamp(2.4rem,5vw,4rem)", fontWeight:900, color:"#fff", letterSpacing:"-.03em", lineHeight:1.08, marginBottom:16 }}>Clothes</h1>
          <p style={{ color:"rgba(255,255,255,.65)", fontSize:17, maxWidth:500, lineHeight:1.7 }}>
            Discover our premium collection of clothing, crafted with refined detail for modern style.
          </p>
        </div>
      </div>
          ...
          <div style={{ background:"#fff", borderRadius:20, border:"1px solid #f0f0f0", padding:24, boxShadow:"0 2px 12px rgba(0,0,0,.05)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, paddingBottom:16, borderBottom:"1px solid #f5f5f5" }}>
              <h2 style={{ fontWeight:700, fontSize:15, color:"#111", display:"flex", alignItems:"center", gap:8 }}>
                <SlidersHorizontal style={{ width:16,height:16,color:"#8b5cf6" }} /> Filters
              </h2>
              <button onClick={() => setPriceRange([0,50000])} style={{ fontSize:12, color:"#9ca3af", fontWeight:600, background:"none", border:"none", cursor:"pointer" }}>Clear</button>
            </div>
            ...
            <input type="range" min={0} max={50000} value={priceRange[1]}
              ...
              style={{ width:"100%", accentColor:"#8b5cf6", marginBottom:14 }} />
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {sorted.length > 0 ? (
                  ...
                  <div key={product._id} className="pp-card"
                    ...
                    <div style={{ position:"relative", aspectRatio:"1/1", overflow:"hidden", background:"#f8f8f8" }}>
                      ...
                      <div style={{ position:"absolute",top:10,left:10,display:"flex",flexDirection:"column",gap:5,zIndex:10 }}>
                        {product.onSale && <span style={{ background:"linear-gradient(135deg,#ef4444,#f97316)",color:"#fff",fontSize:10,fontWeight:700,padding:"3px 9px",borderRadius:999,letterSpacing:".06em" }}>SALE</span>}
                        {product.isNew && !product.onSale && <span style={{ background:"linear-gradient(135deg,#10b981,#34d399)",color:"#fff",fontSize:10,fontWeight:700,padding:"3px 9px",borderRadius:999,letterSpacing:".06em" }}>NEW</span>}
                      </div>
                      {/* Wishlist */}
            ...
            <div style={{ textAlign:"center", padding:"80px 24px", background:"#fff", borderRadius:20, border:"1px solid #f0f0f0" }}>
              ...
              <h3 style={{ fontWeight:700, fontSize:18, color:"#111", marginBottom:8 }}>No clothing items found</h3>
