...
const Accessories = () => {
  ...
  const sorted = useMemo(() => {
    ...
    if (sortBy === "price-low") return [...filtered].sort((a,b) => (a.onSale&&a.salePrice?a.salePrice:a.price) - (b.onSale&&b.salePrice?b.salePrice:b.price));
    if (sortBy === "price-high") return [...filtered].sort((a,b) => (b.onSale&&b.salePrice?b.salePrice:b.price) - (a.onSale&&a.salePrice?a.salePrice:a.price));
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#fafafa" }}>
      <div style={{ textAlign:"center" }}>
        <div style={{ width:52,height:52,borderRadius:"50%",border:"4px solid #e5e7eb",borderTopColor:"#111",animation:"spin 1s linear infinite",margin:"0 auto 14px" }} />
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#fafafa" }}>
      ...
      <div style={{
        ...
        background:"linear-gradient(135deg, #1e0a3c 0%, #3b0764 50%, #1a0a2e 100%)",
        padding:"72px 24px 80px",
      }}>
        <div style={{ position:"absolute", top:-80, right:-80, width:360, height:360, background:"radial-gradient(circle,rgba(168,85,247,.22) 0%,transparent 70%)", borderRadius:"50%", pointerEvents:"none" }} />
        <div style={{ position:"absolute", bottom:-60, left:-60, width:280, height:280, background:"radial-gradient(circle,rgba(99,102,241,.16) 0%,transparent 70%)", borderRadius:"50%", pointerEvents:"none" }} />
        <div style={{ maxWidth:1200, margin:"0 auto", position:"relative", zIndex:1 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(168,85,247,.15)", border:"1px solid rgba(168,85,247,.3)", color:"#d8b4fe", padding:"6px 16px", borderRadius:999, fontSize:12, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", marginBottom:20 }}>
            ✦ Refined Details
          </div>
          <h1 style={{ fontSize:"clamp(2.4rem,5vw,4rem)", fontWeight:900, color:"#fff", letterSpacing:"-.03em", lineHeight:1.08, marginBottom:16 }}>Accessories</h1>
          <p style={{ color:"rgba(255,255,255,.65)", fontSize:17, maxWidth:500, lineHeight:1.7 }}>
            Complete your look with premium accessories — from statement pieces to everyday elegance.
          </p>
        </div>
      </div>
          ...
          <div style={{ background:"#fff", borderRadius:20, border:"1px solid #f0f0f0", padding:24, boxShadow:"0 2px 12px rgba(0,0,0,.05)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, paddingBottom:16, borderBottom:"1px solid #f5f5f5" }}>
              <h2 style={{ fontWeight:700, fontSize:15, color:"#111", display:"flex", alignItems:"center", gap:8 }}>
                <SlidersHorizontal style={{ width:16,height:16,color:"#a855f7" }} /> Filters
              </h2>
              <button onClick={() => setPriceRange([0,1000000])} style={{ fontSize:12, color:"#9ca3af", fontWeight:600, background:"none", border:"none", cursor:"pointer" }}>Clear</button>
            </div>
            <p style={{ fontSize:12, fontWeight:700, color:"#374151", marginBottom:12, textTransform:"uppercase", letterSpacing:".06em" }}>Price Range</p>
            <input type="range" min={0} max={1000000} value={priceRange[1]}
              onChange={e => setPriceRange([0, +e.target.value])}
              style={{ width:"100%", accentColor:"#a855f7", marginBottom:14 }} />
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", background:"#fff", border:"1px solid #f0f0f0", borderRadius:16, padding:"14px 20px", marginBottom:28, boxShadow:"0 2px 8px rgba(0,0,0,.04)" }}>
            <span style={{ fontSize:14, color:"#6b7280", fontWeight:500 }}>
              <strong style={{ color:"#111" }}>{sorted.length}</strong> products
            </span>
          ...
          {sorted.length > 0 ? (
                  ...
                  <div key={product._id} className="pp-card"
                    ...
                    <div style={{ position:"relative", aspectRatio:"1/1", overflow:"hidden", background:"#f8f8f8" }}>
                        ...
                        {images.map((img,i) => (
                          <SwiperSlide key={i} style={{height:"100%"}}>
                            <img className="pp-card-img" src={img} alt={product.name}
                              ...
                              onError={e => { e.target.src="https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&h=500&fit=crop"; }} />
                          </SwiperSlide>
                        ))}
                      </Swiper>
                      <div style={{ position:"absolute",top:10,left:10,display:"flex",flexDirection:"column",gap:5,zIndex:10 }}>
                        {product.onSale && <span style={{ background:"linear-gradient(135deg,#ef4444,#f97316)",color:"#fff",fontSize:10,fontWeight:700,padding:"3px 9px",borderRadius:999 }}>SALE</span>}
                        {product.isNew && !product.onSale && <span style={{ background:"linear-gradient(135deg,#10b981,#34d399)",color:"#fff",fontSize:10,fontWeight:700,padding:"3px 9px",borderRadius:999 }}>NEW</span>}
                      </div>
                      ...
                      <div className="pp-card-actions" style={{ position:"absolute",bottom:10,left:10,right:10,display:"flex",gap:8,zIndex:10 }}>
                        ...
                        <button onClick={e => { e.stopPropagation(); navigate(`/product/${product._id}`); }}
                          style={{ flex:1,background:"#111",color:"#fff",border:"none",borderRadius:10,padding:"9px 0",fontWeight:600,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",gap:5,cursor:"pointer" }}>
                      ...
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        <div>
                          <span style={{ fontSize:18,fontWeight:800,color:"#111" }}>
                            Rs. {product.onSale && product.salePrice ? product.salePrice.toLocaleString() : product.price.toLocaleString()}
                          </span>
            ...
            <div style={{ textAlign:"center", padding:"80px 24px", background:"#fff", borderRadius:20, border:"1px solid #f0f0f0" }}>
              ...
              <h3 style={{ fontWeight:700, fontSize:18, color:"#111", marginBottom:8 }}>No accessories found</h3>
              <p style={{ color:"#9ca3af", fontSize:14 }}>Try clearing or expanding your filters.</p>
            </div>
          )}
        </div>
      </div>
