...
const Bags = () => {
  ...
  const getProductImages = (product) => {
    if (!product.image || product.image.length === 0)
      return ["https://images.unsplash.com/photo-1584917865442-de89df76afd3?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80"];
  ...
  const sorted = [...bagsProducts].sort((a, b) => {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#fafafa" }}>
      ...
      <div style={{
        ...
        background:"linear-gradient(135deg, #0a2018 0%, #0f3d2a 50%, #082016 100%)",
        padding:"72px 24px 80px",
      }}>
        <div style={{ position:"absolute", top:-80, right:-80, width:360, height:360, background:"radial-gradient(circle,rgba(16,185,129,.2) 0%,transparent 70%)", borderRadius:"50%", pointerEvents:"none" }} />
        <div style={{ position:"absolute", bottom:-60, left:-60, width:280, height:280, background:"radial-gradient(circle,rgba(52,211,153,.14) 0%,transparent 70%)", borderRadius:"50%", pointerEvents:"none" }} />
        <div style={{ maxWidth:1200, margin:"0 auto", position:"relative", zIndex:1 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(16,185,129,.15)", border:"1px solid rgba(16,185,129,.3)", color:"#6ee7b7", padding:"6px 16px", borderRadius:999, fontSize:12, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", marginBottom:20 }}>
            ✦ Artisanal Craftsmanship
          </div>
          <h1 style={{ fontSize:"clamp(2.4rem,5vw,4rem)", fontWeight:900, color:"#fff", letterSpacing:"-.03em", lineHeight:1.08, marginBottom:16 }}>Bags</h1>
          <p style={{ color:"rgba(255,255,255,.65)", fontSize:17, maxWidth:500, lineHeight:1.7 }}>
            Carry your essentials in style — blending function and modern aesthetics with expert craftsmanship.
          </p>
        </div>
      </div>
          ...
          <div style={{ background:"#fff", borderRadius:20, border:"1px solid #f0f0f0", padding:24, boxShadow:"0 2px 12px rgba(0,0,0,.05)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, paddingBottom:16, borderBottom:"1px solid #f5f5f5" }}>
              <h2 style={{ fontWeight:700, fontSize:15, color:"#111", display:"flex", alignItems:"center", gap:8 }}>
                <SlidersHorizontal style={{ width:16,height:16,color:"#10b981" }} /> Filters
              </h2>
            ...
            <input type="range" min={0} max={50000} value={priceRange[1]}
              ...
              style={{ width:"100%", accentColor:"#10b981", marginBottom:14 }} />
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {sorted.length > 0 ? (
                        ...
                        {images.map((img,i) => (
                          <SwiperSlide key={i} style={{height:"100%"}}>
                            <img className="pp-card-img" src={img} alt={product.name}
                              ...
                              onError={e => { e.target.src="https://images.unsplash.com/photo-1584917865442-de89df76afd3?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80"; }} />
                          </SwiperSlide>
                        ))}
                      </Swiper>
            ...
            <div style={{ textAlign:"center", padding:"80px 24px", background:"#fff", borderRadius:20, border:"1px solid #f0f0f0" }}>
              ...
              <h3 style={{ fontWeight:700, fontSize:18, color:"#111", marginBottom:8 }}>No bags found</h3>
